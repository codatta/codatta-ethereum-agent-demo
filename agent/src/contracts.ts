...
import { addresses, deployer, client, evaluator } from "./config.js";

// --- ABIs (minimal, only what the demo needs) ---
...
// --- Contract instances per role ---

export function getContracts() {
  return {
    // Deployer-connected (agent owner)
    identity: new ethers.Contract(addresses.identityRegistry, IdentityRegistryABI, deployer),
    validationAsOwner: new ethers.Contract(addresses.validationRegistry, ValidationRegistryABI, deployer),

    // Client-connected
    acpAsClient: new ethers.Contract(addresses.acpContract, ACPContractABI, client),
    tokenAsClient: new ethers.Contract(addresses.tokenContract, MockERC20ABI, client),
    reputationAsClient: new ethers.Contract(addresses.reputationRegistry, ReputationRegistryABI, client),

    // Evaluator-connected
    acpAsEvaluator: new ethers.Contract(addresses.acpContract, ACPContractABI, evaluator),
    validationAsEvaluator: new ethers.Contract(addresses.validationRegistry, ValidationRegistryABI, evaluator),

    // Read-only
    token: new ethers.Contract(addresses.tokenContract, MockERC20ABI, deployer),
    reputation: new ethers.Contract(addresses.reputationRegistry, ReputationRegistryABI, deployer),
    validation: new ethers.Contract(addresses.validationRegistry, ValidationRegistryABI, deployer),
    acpRead: new ethers.Contract(addresses.acpContract, ACPContractABI, deployer),
  };
}
