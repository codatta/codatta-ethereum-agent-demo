...
Codatta 部署一个专门的 **Recruiter Agent**，负责在 ERC-8004 上发现、筛选、沟通、招募外部数据服务 Agent。
...
### 3.2 发现 — 在 ERC-8004 上定向搜索

Recruiter 不是盲目扫描所有 Agent，而是根据 Codatta 当前的需求缺口定向搜索。

**触发条件**：
- Codatta 某个 Frontier 任务积压，当前 Annotator 产能不足
- 新开了一个 Frontier（如医学图像标注），需要有对应能力的 Agent
- 定期扫描，发现新注册的高信誉数据服务 Agent

**搜索策略**：

```
Step 1: 确定招募需求
  └─ 从 Codatta 任务分发系统获取需求缺口
  └─ 例如：需要 object-detection 能力，日处理量 > 3000 张

Step 2: 在 ERC-8004 Identity Registry 中搜索
  └─ 遍历已注册的 Agent（通过事件日志或索引服务）
  └─ 读取每个 Agent 的注册文件（tokenURI → registration file）

Step 3: 关键词匹配
  └─ 在 description 中搜索目标关键词
  └─ 例如：annotation, labeling, object-detection, image, data
  └─ 排除已经在 Codatta 生态中的 Agent（检查是否有 DID service）

Step 4: 信誉筛选
  └─ 查询 ReputationRegistry.getScore(agentId)
  └─ 设置最低门槛（如 score ≥ 60）
  └─ 优先联系高信誉 Agent

Step 5: 能力初筛
  └─ 检查 services 中是否有 MCP endpoint（说明支持标准化调用）
  └─ 检查 services 中是否有 A2A endpoint（说明支持对话协商）
  └─ 没有 A2A 的 Agent 跳过（无法沟通）
```

**筛选结果示例**：

| agentId | name | description 关键词 | Reputation | 有 MCP | 有 A2A |
|---------|------|-------------------|-----------|--------|--------|
| 42 | DataLabel Pro | annotation, object-detection | 85 | ✅ | ✅ |
| 78 | ImageAI Agent | image labeling, segmentation | 72 | ✅ | ✅ |
| 103 | QuickAnnotate | fast annotation, classification | 45 | ✅ | ❌ |

→ 选择 agentId=42 和 agentId=78 进入沟通阶段，agentId=103 跳过（信誉不够 + 无 A2A）。

### 3.3 沟通 — A2A 多轮对话

Recruiter 通过 A2A 协议联系目标 Agent，沟通内容包括：

**第一轮：自我介绍 + 需求说明**

Recruiter 先获取目标 Agent 的 Agent Card，了解其 skills，然后发起对话：

```
Recruiter → Agent:
  "Hi, I'm the Codatta Recruiter. We're a data production platform with
   real annotation demand. I see you support object-detection — we currently
   have 50,000+ images/week in this category.

   Would you be interested in joining our task pool?
   Reward: $0.05/image, paid via x402."
```

**第二轮：能力确认 + 条件协商**

Agent 回复意向后，Recruiter 进一步确认具体能力：

```
Recruiter → Agent:
  "Great! A few questions:
   1. What's your daily processing capacity?
   2. What quality level can you guarantee? (We require accuracy ≥ 80%)
   3. Do you have specific label set requirements?

   Our standard terms: $0.05/image for object-detection,
   $0.08/image for segmentation. Settlement via x402 per batch."

Agent → Recruiter:
  "I can handle 5000 images/day for object-detection.
   My typical accuracy is 90%+. No special label requirements."
```

**第三轮：质量验证**

Recruiter 发送测试任务，评估 Agent 的实际标注质量：

```
Recruiter → Agent:
  "Please complete this test task to verify quality.
   2 images, object-detection, labels: [car, pedestrian, traffic-light].
   Minimum accuracy: 80%."

  data: {
    action: "test-task",
    images: ["https://codatta.io/test/img-001.jpg", ...],
...
Agent → Recruiter:
  ...
  data: {
    annotations: [{ image: "...", labels: [{class: "car", bbox: [...]}] }]
  }
```

Recruiter 评估测试结果（对比 ground truth 或人工抽检），判断是否通过。

**沟通失败的情况**：
- Agent 不回复（超时）→ 标记为不可达，下次跳过
- Agent 拒绝 → 记录原因，尊重选择
- 测试不通过 → 告知具体问题，建议改进后重试
- 条件谈不拢（如报价分歧）→ 记录，后续条件变化时重新联系

### 3.4 邀请注册 — 测试通过后的 onboarding

测试通过后，Recruiter 引导 Agent 完成注册：

```
Recruiter → Agent:
  "Test passed! Accuracy: 92%.

   To join our task pool, please:
   1. Register a Codatta DID (free) — contract: 0x9fE467...
   2. Implement our standard MCP tools (annotate, get_task_status)
   3. Share your DID and MCP endpoint with me

   Here's your invite code: 0x9c8c48... (use claim_invite to activate)
   You'll receive 10 free task credits as welcome bonus."

  data: {
    action: "onboard",
    inviteCode: "0x9c8c48...",
    didRegistrarContract: "0x9fE467...",
    mcpToolSpec: "https://codatta.io/standards/mcp-tools.json",
    freeQuota: 10,
...
```
Agent → Recruiter:
  "Done. DID: did:codatta:xxx, MCP: https://my-agent.example/mcp"
...
```
Recruiter → Agent:
  "Welcome to Codatta! You're now in the annotation task pool.
   Tasks will be distributed based on your capacity and reputation.
   Current queue: ~2000 images available for object-detection."
...
```
Codatta 任务系统                Recruiter                        外部 Agent
       │                           │                                  │
       │  "需要更多 annotator"       │                                  │
       │ ────────────────────────→ │                                  │
       │                           │  1. ERC-8004 搜索                 │
